...
def _(pd):
    def compare_readwise(kmer):
        ...
        readwise = pd.read_csv(
            f"assets/SI3_SI4/{kmer}/simadna_md_1000_Ziphiidae_{kmer}_readwise", sep=","
        )
...
def _(compare_readwise, mo, pd):
    ...
    for kmer in mo.status.progress_bar(
        [f"kmer{x}" for x in [18, 19, 20, 21, 22, 24, 28, 30]]
    ):
        _tmp = pd.read_csv(
...
def _(df_full):
    ...
    df_full["ProportionExpectedBreadth"] = df_full.ProportionExpectedBreadth.astype(
        float
    )
    return
...
@app.cell
def _():
    damage_profiles_hr = {"md": "Medium Damage", "hd": "High Damage"}
...
def _(damage_profiles_hr, df_full, re):
    ...
    kmer_data = (
        ...
        .groupby(["RG", "Kmer"], as_index=False)
        .sum()
    )
...
def _(damage_profiles_hr, df_full, re):
    ...
    kmer_data_filtered = (
        df_full[
            (df_full.FamPercentage > 0.5) & (df_full.ProportionExpectedBreadth > 0.5)
        ][["RG", "Kmer", "All", "True_Positive", "False_Positive", "FamPercentage"]]
        .groupby(["RG", "Kmer"], as_index=False)
        .sum()
    )

    kmer_data_filtered["Accuracy"] = (
        100 * kmer_data_filtered["True_Positive"] / kmer_data_filtered["All"]
    )
...
def _(get_palette, kmer_data, kmer_data_filtered, plt, sns):
    ...
    _fig, _axes = plt.subplot_mosaic("AB", figsize=(10, 5), sharey=True)
    _ax1 = _axes["A"]
    _ax2 = _axes["B"]
    ...
    sns.move_legend(_ax2, "lower left")
...
def _(damage_profiles_hr, df_full, re):
    ...
    df_full["percentage_mt"] = df_full.RG.apply(
        lambda x: re.search("[01]+", x).group() if bool(re.search("[01]+", x)) else "1"
    )
    return
...
def _(df_full, family_dict, get_palette, mpatches, plt, sns):
    ...
    _sizes = ["0.1", "1", "10", "100", "1000"]

    _percent_mt = ["01", "1", "10", "100", "1000"]
...
@app.cell
def _(df_full, family_dict, get_palette, plt, sns):
    _fig, _axes = plt.subplot_mosaic("ABC", figsize=(12, 5), sharey=True)
    ...
    _data = _subset.groupby(["damage", "percentage_mt", "Kmer"], as_index=False).count()

    _g = sns.barplot(
        data=_data[_data.damage == "No Damage"],
        x="Kmer",
        y='Family',
        hue="percentage_mt",
        ax=_axes['A'],
    ...
    _axes['A'].set_xlabel("")
    _axes['A'].set_ylabel("Number of Families", size=15)
    _axes['A'].set_title("A", size=22)
    _axes['A'].spines["top"].set_visible(False)
    _axes['A'].spines["right"].set_visible(False)

    plt.setp(_axes['A'].get_xticklabels(), rotation=45, ha="right", rotation_mode="anchor")

    _legend = _g.get_legend()
    _legend.set_title('Database Size')
    sns.move_legend(_g, 'upper left')

    ## Medium Damage

    sns.barplot(
        data=_data[_data.damage == "Medium Damage"],
        x="Kmer",
        y='Family',
        hue="percentage_mt",
        ax=_axes['B'],
    ...
    _axes['B'].set_xlabel("")
    _axes['B'].set_title("B", size=22)
    _axes['B'].spines["top"].set_visible(False)
    _axes['B'].spines["right"].set_visible(False)

    plt.setp(_axes['B'].get_xticklabels(), rotation=45, ha="right", rotation_mode="anchor")

    ## High Damage

    sns.barplot(
        data=_data[_data.damage == "High Damage"],
        x="Kmer",
        y='Family',
        hue="percentage_mt",
        ax=_axes['C'],
    ...
    _axes['C'].set_xlabel("")
    _axes['C'].set_title("C", size=22)
    _axes['C'].spines["top"].set_visible(False)
    _axes['C'].spines["right"].set_visible(False)
...
def _(Path, damage_profiles_hr, kmer, mo, pd, re):
    ...
    for _kmer in mo.status.progress_bar(
        [f"kmer{x}" for x in [18, 19, 20, 21, 22, 24, 28, 30]]
    ):
        for _report in Path(f"assets/SI3_SI4/{kmer}/quicksand_v2.3/stats/").glob(
            "*.report"
        ):
...
@app.cell
def _(family_dict, get_palette, kraken, plt, sns):
    _fig, _axes = plt.subplot_mosaic("ABC", figsize=(12, 5), sharey=True)
    ...
    _axes['B'].set_xlabel("")
    _axes['B'].set_ylabel("Number of Families", size=15)
    _axes['B'].set_title("B", size=22)
    _axes['B'].spines["top"].set_visible(False)
    _axes['B'].spines["right"].set_visible(False)
    ...
    _axes['C'].set_xlabel("")
    _axes['C'].set_ylabel("Number of Families", size=18)
    _axes['C'].set_title("C", size=22)
    _axes['C'].spines["top"].set_visible(False)
    _axes['C'].spines["right"].set_visible(False)
...
def _(df_full, mpatches, plot_kmerperformance, plt, sns):
    ...
    _colors = {
        fam: color
        for (fam, color) in zip(
            _families,
            [
                matplotlib.colors.to_rgb(x)
                for x in sns.color_palette("Spectral", len(_families)).as_hex()
            ],
        )
    }
    ...
    _ax1 = plot_kmerperformance(
        df_full[(df_full.damage == "No Damage") & df_full.Family.isin(_families)],
        title="A",
        ax=_ax1,
        colors=_colors,
    )
    ...
    _ax2 = plot_kmerperformance(
        df_full[(df_full.damage == "Medium Damage") & df_full.Family.isin(_families)],
        title="B",
        ax=_ax2,
        colors=_colors,
        ylabel=False,
    ...
    _ax3 = plot_kmerperformance(
        df_full[(df_full.damage == "High Damage") & df_full.Family.isin(_families)],
        title="C",
        ax=_ax3,
        colors=_colors,
        ylabel=False,
...
def _(df_full, plt, sns):
    def plot_kmerperformance_merged(
        ...
        ax = sns.boxplot(data=fam, x="Kmer", y="relative_best", ax=ax, **kwargs)

        plt.setp(ax.get_xticklabels(), rotation=45, ha="right", rotation_mode="anchor")

        ax.set_title(title, size=21)
        ax.set_ylabel("Relative Sensitivity", size=15)
        ax.set_xlabel("")
        if not ylabel:
            ax.set_ylabel("")
    ...
    _fig = plt.figure(figsize=(12, 5), dpi=100)
    _ax1 = _fig.add_subplot(131)
    _ax2 = _fig.add_subplot(132, sharey=_ax1)
    _ax3 = _fig.add_subplot(133, sharey=_ax1)
...
def _(df_full, family_dict):
    ...
    df_summary = (
        df_full[df_full.Family.isin(family_dict)]
        .groupby(["damage", "RG", "Kmer", "Family"], as_index=False)[
            ["All", "False_Positive", "True_Positive"]
        ]
        .sum()
    )
