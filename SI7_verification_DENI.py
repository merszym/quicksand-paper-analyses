...
def _(pd):
    # import the data
    data = pd.read_csv(
        "assets/SI7/20250429_Pleistocene sediment DNA reveals hominin and faunal turnovers at Denisova Cave_library_m_1.csv"
    )
    return (data,)
...
def _(pd):
    ...
    familydata = pd.read_csv("assets/SI7/DENI_family_annotation.tsv", sep="\t")
...
def _(Path, data, familydata, pd):
    ...
    for _i, _dat in data[data["Capture Probe"] == "AA75"].groupby(
        ["Sequencing Run", "Sequencing Lane"]
    ):
        _run, _lane = _i
        _p = Path("quicksand_reports/")
        for _run in _p.glob(f"{_run}.{_lane}*.tsv"):
            df = pd.concat([df, pd.read_csv(_run, sep="\t")], ignore_index=True)
...
def _(aa75, aa75_ancient, get_palette, plt, sns, ticker):
    ...
    _hue_order = ["Evidence", "Possible", "Unlikely"]
    ...
    _ax1.axhline(y=0.5, color="red", lw=1, ls="--")
    _ax1.axvline(x=0.5, color="red", lw=1, ls="--")
    ...
    _ax2.axhline(y=0.5, color="red", lw=1, ls="--")
...
def _(aa75_filtered, pd):
    ...
    aa75_filtered["Marker"] = aa75_filtered["Sample Synonyms"].apply(
        lambda x: x.split(";")[1].split(":")[1] if x == x else ""
    )

    samples = aa75_filtered[["Sample Name", "Marker"]].drop_duplicates(
        subset=["Sample Name", "Marker"]
    )
...
def _(aa75_filtered, coord, mpatches, plt):
    ...
    for _n, (_i, _grp) in enumerate(coord.groupby("marker")):
        _x = (
            set(_grp["relx"]).pop() * 0.85 + 0.05
        )  # this is to fit it to the padding of the image
        _y = 1 - set(_grp["rely"]).pop() * 0.88 - 0.08  # same here

        _data = aa75_filtered[aa75_filtered.Marker == _i].copy()
        _colors = [
            _colors_dict[_x] if _x in _colors_dict else "grey" for _x in _data["Family"]
        ]
        ...
        if _x == _x and _y == _y:
            ...
            _ax.pie(
                [1], colors=["white"], wedgeprops={"edgecolor": "k"}
            )  # plot an empty white pie
            _ax.pie(
                _sizes,
                colors=_colors,
                startangle=-270,
...
@app.cell
def _(pd):
    megan_aa75 = pd.read_excel(
        "assets/SI7/zavala2021_supplements.xlsx",
        sheet_name="Mammalian MtDNA Summary",
        skiprows=2,
    )
    megan_aa75.drop(
        [
...
def _(get_palette, megan_aa75, pd, plt, qsaa75, sns, ticker):
    ...
    _comp = pd.concat([megan_aa75, qsaa75], ignore_index=True)
...
def _(megan_aa75, pd, qsaa75, re):
    ...
    _dict = {
        k: (re.search("SP[0-9]+", v).group() if v == v else v)
        for k, v in zip(_trans["Sample Name"], _trans["Sample Synonyms"])
    }
    ...
    print(
        Counter(
            _comp[
                (_comp.origin_x == _comp.origin_x) & (_comp.origin_y == _comp.origin_y)
            ]["Family"]
        )
    )
    print(
        len(
            set(
                _comp[
                    (_comp.origin_x == _comp.origin_x)
                    & (_comp.origin_y == _comp.origin_y)
                ]["SampleID"]
            )
        )
    )
...
def _(aa163, pd):
    ...
    zav2021 = zav2021[
        zav2021.apply(lambda x: compare_lib_counts(x["Library"], x["ReadsRaw"]), axis=1)
    ].copy()
    return (zav2021,)
...
def convert_dataframe(df, style="quicksand", type="deam"):
    ...
    aa163 = aa163.melt(
        id_vars=cols, value_vars=[x for x in aa163.columns if x.endswith(type)]
    )
    ...
    aa163 = aa163[
        (
            aa163.apply(
                lambda x: x["variable"].startswith("H_")
                and (
                    x["H_support_deam"].startswith("^^^")
                    or x["H_support_deam"].startswith("**")
                ),
                axis=1,
            )
        )
...
def _(MIN_SUPPORT, coord, mpatches, plt, re):
    def print_map(df, legend=True):
        ...
        for _n, (_i, _grp) in enumerate(coord.groupby("marker")):
            ...
            _y = 1 - set(_grp["rely"]).pop() * 0.88 - 0.07  # same here
            ...
            if _x == _x and _y == _y:
                ...
                _ax.pie(
                    [1], colors=["white"], wedgeprops=_wedgeprops
                )  # plot an empty white pie for negative

                # now check if
                _data["support"] = _data["value"].apply(
                    lambda x: int(re.search("[0-9]+", x).group())
                )
                _data["sizes"] = _data["value"].apply(
                    lambda x: int(re.search("(?<=\()[0-9]+", x).group())
                )

                _data = _data[_data.support > MIN_SUPPORT].copy()
                _data = _data.groupby("variable", as_index=False).mean(
                    numeric_only=True
                )  # check how elena did it. Take any positive?

                if "sizes" in _data.columns:
                    _colors = [
                        _colors_dict[x] if x in _colors_dict else "grey"
                        for x in _data["variable"]
                    ]
...
def _(TYPE, aa163, coord, print_map):
    ...
    aa163_ancient["Marker"] = aa163_ancient["Sample Synonyms"].apply(
        lambda x: x.split(";")[1].split(":")[1] if x == x else ""
    )

    aa163_ancient = aa163_ancient.merge(
        coord, left_on="Marker", right_on="marker", how="left", validate="m:1"
    )
...
def _(TYPE, coord, print_map, zav2021):
    ...
    zav2021_ancient = zav2021_ancient.merge(
        coord, left_on="Marker", right_on="marker", how="left", validate="m:1"
    )

    print_map(zav2021_ancient, legend=False)
    return
...
def _(aa163_filtered, pd, zav2021):
    ...
    hum_combined = pd.concat([zav2021, aa163_filtered], ignore_index=True)
...
def _(aa163_filtered, get_palette, plt, venn2, venn2_circles, zav2021):
    _count_ancient = (
        aa163_filtered[["Library", "MM_Ancient"]]
        .merge(zav2021[["Library", "Ancient"]], on="Library", how="left", validate="1:1")
        .copy()
    )
    _count_ancient["MM_Ancient"] = _count_ancient.MM_Ancient.apply(
        lambda x: "Yes" if x == "++" else "No"
    )

    _ancient_in_zav_AND_quicksand = len(
        _count_ancient[
            _count_ancient.apply(lambda x: x["Ancient"] == x["MM_Ancient"], axis=1)
        ]["Library"]
    )
    _ancient_in_zav_ONLY = len(
        _count_ancient[
            _count_ancient.apply(
                lambda x: x["Ancient"] == "Yes" and x["MM_Ancient"] == "No", axis=1
            )
        ]["Library"]
    )
    _ancient_in_quicksand_ONLY = len(
        _count_ancient[
            _count_ancient.apply(
                lambda x: x["Ancient"] == "No" and x["MM_Ancient"] == "Yes", axis=1
            )
        ]["Library"]
    )
    ...
    _v = venn2(
        subsets=(
            _ancient_in_zav_ONLY,
            _ancient_in_quicksand_ONLY,
            _ancient_in_zav_AND_quicksand,
        ),
    ...
    _v.get_label_by_id("01").set_text(
...
def _(aa163_filtered):
    ...
    # it turned from ++ to +
    aa163_filtered[aa163_filtered.Library == "Lib.G.783"][
        [
            "MM_Ancient",
            "5'CT(95%CI)",
            "3'CT(95%CI)",
...
def _(aa163_all_der):
    ...
    aa163_positions = aa163_positions[
        aa163_positions.positions.apply(lambda x: x == x and x != "")
    ].copy()
    aa163_positions["pos"] = aa163_positions["positions"].apply(
        lambda x: int(x.replace("(", "").split(":")[0])
    )
    aa163_positions["count"] = aa163_positions["positions"].apply(
...
def _(aa163_positions, filterpos):
    aa163_filteredpos = aa163_positions[
        aa163_positions.pos.isin(filterpos) == False
    ].copy()
    ...
    aa163_filteredpos["Marker"] = aa163_filteredpos["Sample Synonyms"].apply(
        lambda x: x.split(";")[1].split(":")[1] if x == x else ""
    )
    aa163_filteredpos["position"] = aa163_filteredpos.apply(
        lambda x: f"({x['pos']}:{x['count']})", axis=1
    )
    aa163_filteredpos = aa163_filteredpos.groupby(
        ["Library", "Marker", "variable"], as_index=False
    ).sum()
    aa163_filteredpos["n_pos"] = aa163_filteredpos.position.apply(
        lambda x: sum(1 for y in x if y == "(")
    )
    ...
    aa163_filteredpos = aa163_filteredpos[
        ["Library", "Marker", "variable", "position", "count", "n_pos"]
    ].copy()
...
def _(aa163_all, aa163_filteredpos, re):
    ...
    aa163_all["Marker"] = aa163_all["Sample Synonyms"].apply(
        lambda x: x.split(";")[1].split(":")[1] if x == x else ""
    )
    aa163_newposmerged = aa163_all.merge(
        aa163_filteredpos,
        on=["Library", "Marker", "variable"],
        how="left",
        validate="1:1",
    )

    aa163_newposmerged["overlapping"] = aa163_newposmerged["value"].apply(
        lambda x: int(re.search(r"(?<=\/.)[0-9]+", x).group())
    )
    aa163_newposmerged["initial"] = aa163_newposmerged["value"].apply(
        lambda x: int(re.search(r"(?<=\()[0-9]+", x).group())
    )
    ...
    aa163_newposmerged["adjusted_overlapping"] = aa163_newposmerged.apply(
        lambda x: x["overlapping"]
        if x["initial"] == x["count"]
        else x["overlapping"] - (x["initial"] - x["count"]),
        axis=1,
    )
    aa163_newposmerged["support"] = aa163_newposmerged.apply(
        lambda x: 100 * round(x["count"] / x["adjusted_overlapping"], 2), axis=1
    )
    aa163_newposmerged["sig"] = aa163_newposmerged.apply(
        lambda x: get_sig_marker(x), axis=1
    )

    ## now assemble new 'values'
    aa163_newposmerged["value"] = aa163_newposmerged.apply(
        lambda x: f"{x['sig']}{x['support']}% ({x['count']} / {x['adjusted_overlapping']})",
        axis=1,
    )
